...
def test_ecology_scene_uses_current_motion_physiology_and_trauma_bundles() -> None:
    ...
    for path in ("cellular_motion/v12", "cellular_physiology/v11", "cellular_trauma/v8", "cellular_ecology/v6"):
...
def test_godot_ecology_smoke_proves_differentiated_metabolism_and_resource_seeking() -> None:
    report = json.loads((ROOT / "outputs/cellular_ecology_godot_report_v5.json").read_text(encoding="utf-8"))
    assert report["format"] == "nullvector-cellular-ecology-godot-smoke-v5" and report["passed"] is True
