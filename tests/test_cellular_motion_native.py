...
RUNTIME = ROOT / "game/generated/cellular_motion/v5"
...
def test_native_scene_uses_live_organ_drivers_not_sprite_frames() -> None:
    ...
    assert 'motion_catalog_path = "res://generated/cellular_motion/v5/motion_catalog.json"' in scene
...
def test_native_smoke_exhausts_motion_programs_and_actuates_damageable_body() -> None:
    report = json.loads((ROOT / "outputs/cellular_physiology_motion_godot_report_v5.json").read_text(encoding="utf-8"))
    assert report["format"] == "nullvector-cellular-motion-godot-smoke-v5"
