...
`outputs/creature_stage_neural_grasper_v1/production_v3_physical_feeder` is the accepted neural controller. It contains an 8,010,637-parameter, 16.0 MB BF16 runtime selected from the EMA weights. On 960 held-out manipulation cases it reached 94.781% appendage accuracy, 0.989 engagement F1, 0.0169 normalized reach MAE, 0.0142 force MAE, 0.0273 brace MAE, perfect target-type and release classification, and 0.0149 throw-impulse MAE. Every declared gate passed. Unlike v2, its learned feeder targets are the exact physical feeder-cell centroids used by collision ingestion. The earlier `production_v1` experiment is retained locally as failed evidence; v2 is retained as an accepted controller experiment but is superseded for physical feeding.
