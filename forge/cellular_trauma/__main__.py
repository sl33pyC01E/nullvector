...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Compile deterministic cellular trauma overlays")
    ...
    if args.command == "build": result = build_bank(args.source, args.output)
    elif args.command == "validate": result = validate_bank(args.manifest)
    else: result = replay_bank(args.manifest)
