...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Apply a soft bilateral prior to cellular offspring")
    commands = parser.add_subparsers(dest="command", required=True)
    build = commands.add_parser("build")
    build.add_argument("--breeding", type=Path, default=DEFAULT_SOURCE)
    ...
    if args.command == "build":
        result = build_bank(args.breeding, args.output)
