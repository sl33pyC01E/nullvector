...
def train(output: Path, corpus: Path, warm_start: Path, *, train_sessions, validation_sessions, test_sessions, training=TrainingConfig(), resume: Path | None = None, stop_after_step: int | None = None):
    ...
    train_actor_change = np.abs(train_group["target_actor_state"].astype(np.float32) - train_group["actor_state"].astype(np.float32)) > 1e-3
    actor_change_frequency = train_actor_change.mean(0, dtype=np.float64)
    actor_gate_pos_weight = np.clip((1.0 - actor_change_frequency) / np.maximum(actor_change_frequency, 1e-8), 1.0, 128.0).astype(np.float32)
    ...
    actor_gate_pos_weight_tensor = torch.from_numpy(actor_gate_pos_weight).to(device).view(1, -1)
    field_gate_pos_weight_tensor = torch.from_numpy(field_gate_pos_weight).to(device).view(1, -1, 1, 1)
    ...
    for step in range(start_step + 1, segment_end + 1):
        ...
        with torch.autocast(device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
            predicted, predicted_actor, predicted_field, gate, _, gate_logits, actor_gate, actor_gate_logits, field_gate, field_gate_logits = model.edit(noisy, previous, torch.zeros(len(indices), device=device), action, control, state, actor, field, previous_action, previous_control)
            ...
            actor_gate_bce = F.binary_cross_entropy_with_logits(actor_gate_logits.float(), actor_change, pos_weight=actor_gate_pos_weight_tensor)
            actor_intersection = (actor_gate * actor_change).sum(1); actor_gate_dice = 1 - ((2 * actor_intersection + 1) / (actor_gate.sum(1) + actor_change.sum(1) + 1)).mean()
            actor_gate_loss = actor_gate_bce + actor_gate_dice
            field_gate_bce = F.binary_cross_entropy_with_logits(field_gate_logits.float(), field_mask, pos_weight=field_gate_pos_weight_tensor)
            ...
            loss = latent_loss + training.actor_state_weight * actor_loss + training.actor_field_weight * field_loss + ACTOR_STATE_GATE_WEIGHT * actor_gate_loss + ACTOR_FIELD_GATE_WEIGHT * field_gate_loss + training.gate_weight * (gate_bce + gate_dice) + training.leakage_weight * leakage + training.contrastive_weight * contrastive
