...
def build(output:Path=DEFAULT_OUTPUT):
    ...
    runtime=CompositeWorldRuntime.from_release(device="cpu");parameters=sum(sum(p.numel() for p in module.parameters()) for module in (runtime.dit,runtime.vae,runtime.refiner,runtime.actor,runtime.organism.model,runtime.physiology.model));del runtime
    payload={"format":FORMAT,"status":"composite_neural_foundation_ready","source_sha256":source_sha256(),"components":components,"parameters":parameters,"artifact_bytes":sum(row.get("artifact",{}).get("bytes",0) for row in components),"capabilities":{"action_conditioned_world_latent":True,"world_frame_encode_decode":True,"pixel_refinement":True,"continuous_cell_organism_raster":True,"causal_actor_state":True,"causal_cell_physiology":True,"factorized_teacher_attached":True},"quality":{"action_dit_latent_and_rgb_beat_persistence":True,"actor_state_beats_persistence":True,"organism_cell_vae_promoted":True,"physiology_promoted":True,"bound_refiner_mae_improvement":.4756335679,"runtime_loader_probe":True},"next_stage":{"reverse_distill_specialist_hidden_state":True,"monolithic_student_ready":False,"android_profile_after_student":True}};payload["manifest_sha256"]=hashlib.sha256(canonical(payload)).hexdigest();_atomic(destination,canonical(payload));return validate(output)
