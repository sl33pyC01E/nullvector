...
class NeuralGrasperRuntime:
    def __init__(self, model, device):
        ...
        self._feeder_anchor_cache: dict[str, np.ndarray] = {}

    def _feeder_anchor(self, organism) -> np.ndarray:
        identity = str(organism.identity_sha256)
        if identity not in self._feeder_anchor_cache:
            self._feeder_anchor_cache[identity] = physical_feeder_anchor(LivingBody(organism))
        return self._feeder_anchor_cache[identity]
    ...
    def plan(self, organism, *, target_type: str, goal: str, direction, distance: float, mass: float, cohesion: float, mobility: float, hostility: float = 0, throw: float = 0, attached: bool = False) -> GraspCommand:
        ...
        global_state = np.zeros(10, np.float32); global_state[int(np.argmax(np.asarray(organism.genome.family_mix, dtype=np.float32)))] = 1; traits = np.asarray(organism.genome.traits, np.float32); global_state[5:8] = (traits[3:8].mean(), traits[8:13].mean(), traits[13:18].mean()); global_state[8:10] = self._feeder_anchor(organism)
