...
def _decode(vae, latent, device):
    ...
    with torch.inference_mode():
        for start in range(0, len(latent), 8):
            rows.append(vae.model.decode(latent[start : start + 8].to(device)).float().cpu())
...
def train(output: Path, episodes, vae_checkpoint: Path, *, config=ModelConfig(), training=TrainingConfig()):
    ...
    vae = WorldFrameVAERuntime.from_checkpoint(vae_checkpoint, device=str(device))
    ...
    for step in range(1, training.steps + 1):
        ...
        with torch.autocast(device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
            ...
            loss = residual + training.edge_weight * edge
        loss.backward()
        ...
        if step == 1 or step % 250 == 0 or step == training.steps:
            row = {"step": step, "loss": round(float(loss), 6), "residual": round(float(residual), 6), "edge": round(float(edge), 6)}
            history.append(row)
            print(json.dumps(row), flush=True)
    ema_cpu = {name: value.detach().cpu() for name, value in ema.items()}
    recovery = {"format": CHECKPOINT_FORMAT, "source_sha256": source_sha256(), "corpus_sha256": corpus_sha, "model_config": config_dict(config), "training_config": config_dict(training), "latent_mean": mean.tolist(), "latent_std": std.tolist(), "ema_state": ema_cpu, "ema_sha256": _hash(ema_cpu), "history": history, "status": "trained_pending_evaluation"}
...
def main():
    ...
    print(json.dumps(train(args.output, args.episodes, args.vae, training=TrainingConfig(steps=args.steps, batch_size=args.batch_size)), indent=2))
