...
class PhysiologyState:
    ...
    def _reachable_from(self, starts: np.ndarray) -> np.ndarray:
        reachable = np.zeros(self.cell_count, dtype=bool)
        queue: deque[int] = deque()
        for index in map(int, starts):
            if self.alive[index] and not reachable[index]:
                reachable[index] = True; queue.append(index)
        while queue:
            current = queue.popleft()
            for neighbor, bond_index in self._adjacency[current]:
                if self.bond_alive[bond_index] and self.alive[neighbor] and not reachable[neighbor]:
    ...
    def capacities(self) -> dict[str, float]:
        ...
        for system_id, name in enumerate(SYSTEM_NAMES):
            ...
            reachable = self._reachable_from(np.flatnonzero(cores & self.alive))
            connected = float(weights[members & self.alive & reachable].sum()) / max(total, 1e-8)
    ...
    def step(self, dt: float, *, nutrient_input: float = 0.0) -> dict[str, float]:
        ...
        capacity = self.capacities(); self.nutrients += nutrient_input
        ...
        heal_rate = 0.025 * capacity["immune"] * capacity["circulation"] * min(1.0, self.energy)
        wounded = self.alive & (self.health < self.max_health)
        if bool(wounded.any()) and heal_rate > 0:
            amount = np.minimum(self.max_health[wounded] - self.health[wounded], heal_rate * dt)
