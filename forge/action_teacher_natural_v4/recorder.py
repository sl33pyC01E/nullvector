...
class NaturalPlayRecorder:
    ...
    def append(self, *, frame, state, actor_state, actor_field, visibility, control, action, selected, timeline_event, timeline, counterfactual, tick, episode_step):
        ...
        values=(np.asarray(frame),np.asarray(state,np.float32),np.asarray(actor_state,np.float32),np.asarray(actor_field,np.float16),np.asarray(visibility,np.float16),np.asarray(control,np.float32),np.asarray(timeline,np.float32),np.asarray(counterfactual,np.float32));frame,state,actor_state,actor_field,visibility,control,timeline,counterfactual=values
        if frame.shape != (FRAME_SIZE[1],FRAME_SIZE[0],3) or frame.dtype != np.uint8:raise ValueError("natural teacher frame drifted")
        if state.shape != (STATE_FEATURES,) or actor_state.shape != (ACTOR_FEATURES,) or actor_field.shape != ACTOR_FIELD_SHAPE or visibility.shape != (1,32,32) or control.shape != (4,) or timeline.shape != (3,) or counterfactual.shape != COUNTERFACTUAL_SHAPE:raise ValueError("natural teacher tensor shape drifted")
        if float(visibility.min()) < 0 or float(visibility.max()) > 1:raise ValueError("natural teacher visibility drifted")
        ...
        row=(frame.copy(),state.copy(),actor_state.copy(),actor_field.copy(),visibility.copy(),control.copy(),ACTIONS.index(action),int(selected),int(timeline_event),timeline.copy(),counterfactual.copy(),int(tick),int(episode_step))
    ...
    def finish(self):
        ...
        arrays={name:np.stack(self._rows[name]) for name in ("frame","state","actor_state","actor_field","visibility","control","timeline","counterfactual")};arrays.update({"action":np.asarray(self._rows["action"],np.uint8),"selected":np.asarray(self._rows["selected"],np.int64),"timeline_event":np.asarray(self._rows["timeline_event"],np.uint8),"tick":np.asarray(self._rows["tick"],np.int64),"episode_step":np.asarray(self._rows["episode_step"],np.int32)});arrays={name:arrays[name] for name in ARRAY_NAMES};archive=staging/"trajectory.npz";np.savez_compressed(archive,**arrays)
        manifest={"format":FORMAT,"source_sha256":source_sha256(),"session_id":self.session_id,"world_seed":self.world_seed,"start_tick":self.start_tick,"end_tick":int(arrays["tick"][-1]),"frames":self.frame_count,"frame_size":list(FRAME_SIZE),"actions":list(ACTIONS),"view_contract":{"clean_student_view":True,"diagnostic_overlays":False,"fixed_actor_camera":True,"no_staged_camera_cuts":True,"teacher_capture_precedes_overlay_composite":True,"current_perception_only":True,"unseen_organisms_hidden":True,"visibility_is_explicit_conditioning":True},"arrays_sha256":_array_digest(arrays),"artifact":{"path":archive.name,"bytes":archive.stat().st_size,"sha256":hashlib.sha256(archive.read_bytes()).hexdigest()},"shapes":{name:list(array.shape) for name,array in arrays.items()},"dtypes":{name:str(array.dtype) for name,array in arrays.items()}};manifest["manifest_sha256"]=hashlib.sha256(canonical(manifest)).hexdigest();(staging/"manifest.json").write_bytes(canonical(manifest));os.replace(staging,destination);self.active=False;return destination
...
def validate_trajectory(path: Path):
    ...
    expected={"clean_student_view":True,"diagnostic_overlays":False,"fixed_actor_camera":True,"no_staged_camera_cuts":True,"teacher_capture_precedes_overlay_composite":True,"current_perception_only":True,"unseen_organisms_hidden":True,"visibility_is_explicit_conditioning":True}
    ...
    if arrays["visibility"].shape != (manifest["frames"],1,32,32) or arrays["visibility"].dtype != np.float16 or float(arrays["visibility"].min()) < 0 or float(arrays["visibility"].max()) > 1:raise ValueError("natural teacher visibility authority drifted")
