...
from .training import prepare_training, rebind_checkpoint, run_supervisor, train_segment
...
def main() -> None:
    ...
    rebind = sub.add_parser("rebind"); rebind.add_argument("--source", type=Path, required=True); rebind.add_argument("--output", type=Path, required=True); rebind.add_argument("--end-step", type=int, required=True)
    ...
    elif args.command == "rebind": result = rebind_checkpoint(args.source, args.output, end_step=args.end_step)
