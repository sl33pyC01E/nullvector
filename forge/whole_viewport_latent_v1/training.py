...
def _load_initializer(path:Path|None,model,model_config):
    ...
    if payload.get("model_config")!=config_dict(model_config):raise ValueError("whole-viewport initializer contract drifted")
    if payload.get("format")==CHECKPOINT_FORMAT:state=payload["state"];kind="release"
    elif "ema" in payload and "model" in payload:state=payload["ema"];kind="segmented_ema"
    else:raise ValueError("whole-viewport initializer format drifted")
    model.load_state_dict(state)
    return {"kind":kind,"path":str(path),"sha256":hashlib.sha256(path.read_bytes()).hexdigest(),"source_sha256":payload.get("source_sha256"),"step":payload.get("step")}
