...
def evaluate_checkpoint(checkpoint: Path, output: Path, *, device_name: str="cuda", samples: int=6, offset: int=0, limit: int|None=None, compile_maps: bool=True) -> dict[str, object]:
    ...
    if samples not in (6,24) or offset<0 or limit is not None and limit<1:raise ValueError("Decoded evaluation selection is invalid.")
    ...
    source_dataset=TopologyProductionDataset(PROJECT_ROOT/"outputs/map_decorator_corpus_v1");refs=source_dataset.evaluation_refs("test",samples);refs=refs[offset:] if limit is None else refs[offset:offset+limit]
    if not refs:raise ValueError("Decoded evaluation selection is empty.")
    ...
    try:
        for source_ref in refs:
            ...
            terrain,hazard,elevation=_decode(codec,sample["tokens"].cpu(),source.config.height,source.config.width);raw=make_raw_topology(terrain,hazard,elevation,shape=(source.config.height,source.config.width));compiled=compile_topology(raw,seed=config.seed,theme=source.theme,config=source.config,start=source.start,exit=source.exit,objectives=source.objectives,spawns=source.spawns) if compile_maps else None
            walk=np.isin(terrain,tuple(WALKABLE_TERRAIN));targets=(source.exit,*source.objectives);repair=None if compiled is None else float(compiled.report["costs"]["repair_fraction"]);name=f"{source.theme}_{source_ref.height}x{source_ref.width}_{len(records):02d}.png";preview=case_preview_png_bytes(source,raw,raw if compiled is None else compiled.data,scale=max(2,min(4,256//max(source_ref.height,source_ref.width))));(staging/name).write_bytes(preview);rows.append((f"{source.theme} {source_ref.width}x{source_ref.height}",preview));records.append({"theme":source.theme,"width":source_ref.width,"height":source_ref.height,"raw_required_reachable":_reachable(walk,source.start,targets),"raw_radius_one_required_reachable":_reachable(_radius_one(walk),source.start,targets),"repair_fraction":repair,"raw_openness":float(walk.mean()),"raw_hazard_fraction":float((hazard!=0).mean()),"unique_tokens":int(torch.unique(sample["tokens"]).numel()),"preview":name})
        repair_values=[row["repair_fraction"] for row in records if row["repair_fraction"] is not None];contact=contact_sheet_png_bytes(rows);(staging/"contact_sheet.png").write_bytes(contact);report={"format":"nullvector-map-topology-prior-v2-decoded-evaluation/1.0.0","checkpoint_sha256":sha256_file(Path(checkpoint)),"step":reported_step,"compiled":compile_maps,"cases":records,"aggregate":{"raw_required_reachable_rate":sum(row["raw_required_reachable"] for row in records)/len(records),"raw_radius_one_required_reachable_rate":sum(row["raw_radius_one_required_reachable"] for row in records)/len(records),"mean_repair_fraction":None if not repair_values else sum(repair_values)/len(repair_values),"mean_unique_tokens":sum(row["unique_tokens"] for row in records)/len(records)},"contact_sheet_sha256":hashlib.sha256(contact).hexdigest()};report["report_sha256"]=hashlib.sha256(canonical_json_bytes(report)).hexdigest();(staging/"evaluation.json").write_bytes(canonical_json_bytes(report));os.replace(staging,output);return report
    finally:
        if staging.exists():
