...
from ..action_teacher_clean_v3 import validate_trajectory
...
RAW_NAMES = ("frame", "state", "actor_state", "actor_field", "control", "action", "selected", "timeline_event", "timeline", "counterfactual", "tick")
...
def build(output=DEFAULT_OUTPUT):
    ...
    if file_sha256(CODEC_CHECKPOINT) != CODEC_SHA256:
        raise ValueError("clean corpus codec drifted")
    runtime = AdaptedWorldFrameCodec.from_checkpoint(CODEC_CHECKPOINT, device="cuda")
    ...
    try:
        for index, name in enumerate(SOURCE_NAMES):
            ...
            if len(raw["frame"]) < 32 or not np.all(np.diff(raw["tick"]) > 0):
                raise ValueError("clean teacher sequence is not strictly contiguous")
            latent = _encode(runtime, raw["frame"])
            path = shard_root / f"{index:04d}-{name}.npz"
            np.savez_compressed(path, latent=latent)
            records.append({
                "index": index, "session_id": name, "frames": len(latent),
                "source": {"path": root.relative_to(SOURCE_ROOT.parent).as_posix(), "manifest_sha256": source["manifest_sha256"], "arrays_sha256": source["arrays_sha256"]},
        ...
        payload = {"format": FORMAT, "source_sha256": source_sha256(), "codec_sha256": CODEC_SHA256, "worlds": len(records), "frames": sum(row["frames"] for row in records), "records": records}
...
def validate(output=DEFAULT_OUTPUT):
    ...
    if raw != canonical(payload) or payload.get("format") != FORMAT or payload.get("source_sha256") != source_sha256() or payload.get("codec_sha256") != CODEC_SHA256:
        raise ValueError("clean corpus manifest drifted")
    unsigned = {key: value for key, value in payload.items() if key != "manifest_sha256"}
    if payload.get("manifest_sha256") != hashlib.sha256(canonical(unsigned)).hexdigest() or payload.get("worlds") != len(SOURCE_NAMES) or len(payload.get("records", ())) != len(SOURCE_NAMES):
        raise ValueError("clean corpus inventory drifted")
    total = 0
    for index, record in enumerate(payload["records"]):
        if record.get("index") != index or record.get("session_id") != SOURCE_NAMES[index]:
            raise ValueError("clean corpus order drifted")
        ...
        if source["manifest_sha256"] != record["source"]["manifest_sha256"] or source["arrays_sha256"] != record["source"]["arrays_sha256"]:
            raise ValueError("clean corpus teacher drifted")
        ...
        if artifact.stat().st_size != record["latent"]["bytes"] or file_sha256(artifact) != record["latent"]["sha256"]:
            raise ValueError("clean corpus latent artifact drifted")
        with np.load(artifact, allow_pickle=False) as archive:
            if archive.files != ["latent"]:
                raise ValueError("clean corpus latent member drifted")
        ...
        if list(latent.shape) != record["latent"]["shape"] or str(latent.dtype) != record["latent"]["dtype"] or latent.shape != (record["frames"], 48, 32, 32) or not np.isfinite(latent).all():
            raise ValueError("clean corpus latent tensor drifted")
        total += len(latent)
    if total != payload.get("frames"):
        raise ValueError("clean corpus frame total drifted")
