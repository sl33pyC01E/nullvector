...
class SpatialWorldActionRuntime:
    ...
    def from_checkpoint(cls, path: Path, *, device: str = "cuda"):
        ...
        if payload.get("format") != CHECKPOINT_FORMAT or payload.get("source_sha256") != source_sha256():
            raise ValueError("spatial world action checkpoint provenance drifted")
        model = SpatialActionDiT(ModelConfig(**payload["model_config"]))
    ...
    def predict_latent(self, current, *, action, control, state):
        ...
        with torch.inference_mode():
            delta = self.model(value, torch.zeros(count, device=self.device), action_tensor, control_tensor, state_tensor)
