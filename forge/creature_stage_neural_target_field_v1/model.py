...
class NeuralGroundedTargetField(nn.Module):
    def __init__(self,config:ModelConfig=ModelConfig()):
        ...
        self.target_head=nn.Sequential(nn.Linear(TARGET_FEATURES+TARGET_OWNER_FEATURES+TARGET_GLOBAL_FEATURES,config.width),nn.SiLU(),
    ...
    def forward(self,owner_state,global_state,owner_mask,muscle_meta,muscle_owner,muscle_mask,target_context):
        ...
        terminal=target_context[:,:,:2].float()+self.target_head(torch.cat((target_context.float(),static_owner,context),-1))*.35
