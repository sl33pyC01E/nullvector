...
from ..creature_stage_developmental_motion.evaluation import (
    _aggregate,
    _render_contact,
    _rollout_specimen,
    _specimen_metrics,
...
def evaluate_checkpoint(
    ...
    *,
    ...
    if runtime_device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("muscle-causal evaluation requested unavailable CUDA")
    model = make_model(CausalActuatorConfig(**contract["model"])).to(runtime_device)
    load_model_state(model, checked["ema_state"])
    model.eval()
    ...
    contact = destination / "evaluation_contact_sheet.png"
    _render_contact(teacher, predictions).save(contact, format="PNG", compress_level=9)
    report: dict[str, Any] = {
        ...
        "checkpoint": {
            "path": project_path(checkpoint_path), "update": update, "sha256": sha256_file(checkpoint_path),
        ...
        "gates": gates,
        "diagnostics": {
            "oracle_previous_target_rmse_px": metrics["copy_previous_rmse_px"],
            "oracle_previous_target_is_noncausal": True,
        },
        "contact_sheet": {
            "path": contact.name, "bytes": contact.stat().st_size,
            "sha256": sha256_file(contact), "visually_inspected": False,
        },
    }
...
def validate_evaluation(destination: Path) -> dict[str, Any]:
    ...
    if errors:
        raise ValueError(f"muscle-causal evaluation schema drifted: {errors[0].message}")
    if (
        report["format"] != EVALUATION_FORMAT or report["source_sha256"] != source_sha256()
        or report["semantic_sha256"] != semantic({key: value for key, value in report.items() if key != "semantic_sha256"})
        or (report["status"] == "passed") != all(report["gates"].values())
        or len(report["specimens"]) != 10
    ):
        raise ValueError("muscle-causal evaluation authority drifted")
    ...
    if contact.stat().st_size != report["contact_sheet"]["bytes"] or sha256_file(contact) != report["contact_sheet"]["sha256"]:
        raise ValueError("muscle-causal evaluation contact sheet drifted")
    checkpoint = PROJECT_ROOT / report["checkpoint"]["path"]
    if sha256_file(checkpoint) != report["checkpoint"]["sha256"]:
        raise ValueError("muscle-causal evaluation checkpoint drifted")
    return {
