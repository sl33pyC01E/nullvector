...
from .contract import DEFAULT_OUTPUT, DEFAULT_PRIOR, DEFAULT_TEACHER, source_sha256
from .evaluation import evaluate_checkpoint, validate_evaluation
from .training import prepare_production, train_next_segment


def main() -> int:
    parser = argparse.ArgumentParser(prog="python -m forge.creature_stage_developmental_actuator_v2")
    ...
    prepare.add_argument("--total-updates", type=int, default=1200)
    prepare.add_argument("--segment-updates", type=int, default=50)
    ...
    validate = subparsers.add_parser("validate-evaluation")
    ...
    if args.command == "source-info":
        ...
        result = prepare_production(
            args.output, teacher_path=args.teacher, prior_path=args.prior,
            total_updates=args.total_updates, segment_updates=args.segment_updates,
            batch_size=args.batch_size,
        )
    elif args.command == "train-next":
        result = train_next_segment(args.output)
    elif args.command == "evaluate":
        result = evaluate_checkpoint(args.output, checkpoint=args.checkpoint, destination=args.destination, device=args.device)
    else:
