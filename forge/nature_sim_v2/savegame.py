...
def save_world(world:NatureWorld,path:Path)->dict:
    ...
    metadata={"format":SAVE_FORMAT,"seed":world.seed,"size":world.size,"max_population":world.max_population,"biome":world.biome,"tick":world.tick_index,"time":world.time,"next_entity":world.next_entity_id,"next_colony":world.next_colony_id,"births":world.births,"deaths":world.deaths,"predation":world.predation_events,"mutations":world.mutation_count,"events":_plain(world.events),"rng":_plain(world.rng.bit_generator.state),"climate_last_event_cycle":world.climate.last_event_cycle,"organisms":organisms,"colonies":[{"id":colony.colony_id,"family":colony.family,"lineage":colony.founder_lineage,"members":sorted(colony.member_ids),"center":colony.center.tolist(),"generation":colony.generation,"fissions":colony.fissions} for colony in sorted(world.colonies.values(),key=lambda item:item.colony_id)],"colony_ecology":colony_ecology,"colony_culture":world.colony_culture.payload(),"ecosystem":world.ecosystem.payload(),"breeding":world.breeding.payload(),"neural_feeding":None if world.feeding_system is None or not hasattr(world.feeding_system,"payload") else _plain(world.feeding_system.payload()),"materials":{"tick":world.materials.tick_index,"next_projectile":world.materials.next_projectile,"projectiles":[asdict(item) for item in world.materials.projectiles]}}
...
def load_world(path:Path,*,motion_policy=None,behavior_policy=None,colony_policy=None,feeding_system=None)->NatureWorld:
    ...
    if feeding_system is not None and metadata.get("neural_feeding") is not None:feeding_system.restore(metadata["neural_feeding"],world)
