...
from ..recurrent_world_runtime_v1 import RecurrentWorldRuntime
...
class NatureDemo:
    ...
    def _compute_neural_raster(self,frame):
        if self.neural_stream is None:
            latent=self.composite.encode(frame);decoded=self.composite.decode(latent)[0].permute(1,2,0).float().cpu().numpy()
        else:
            with torch.cuda.stream(self.neural_stream),torch.autocast("cuda",dtype=torch.bfloat16):latent=self.composite.encode(frame);decoded=self.composite.decode(latent)[0].permute(1,2,0).float().cpu().numpy()
    ...
    def _blit_toroidal(self,surface):
        origin=self.world_to_screen((0,0));tile_w,tile_h=surface.get_size();width,height=self.screen.get_size();x_radius=math.ceil(width/max(1,tile_w))+1;y_radius=math.ceil(height/max(1,tile_h))+1
        for y_index in range(-y_radius,y_radius+1):
            for x_index in range(-x_radius,x_radius+1):self.screen.blit(surface,(int(origin[0]+x_index*tile_w),int(origin[1]+y_index*tile_h)))
    ...
    def _draw_materials(self):
        ...
        scaled=pg.transform.scale(surface,(int(64*self.zoom),int(64*self.zoom)));self._blit_toroidal(scaled)
    ...
    def _draw_perception_fog(self,entity,field):
        pg=self.pg;points=self._perception_polygon(entity,field);center=self.world_to_screen(self._render_position(entity));bubble=max(4,int(field.proximity_range*self.zoom));fog=pg.Surface(self.screen.get_size(),pg.SRCALPHA);fog.fill((1,4,8,248));pg.draw.polygon(fog,(0,0,0,0),points);pg.draw.circle(fog,(0,0,0,0),(int(center[0]),int(center[1])),bubble);self.screen.blit(fog,(0,0));mask=pg.Surface(self.screen.get_size());mask.fill((0,0,0));pg.draw.polygon(mask,(255,255,255),points);pg.draw.circle(mask,(255,255,255),(int(center[0]),int(center[1])),bubble);viewport=mask.subsurface(self._world_viewport_rect());scaled=pg.transform.scale(viewport,(32,32));self.teacher_visibility=np.transpose(pg.surfarray.array3d(scaled)[:,:,0],(1,0))[None].astype(np.float16)/255
    ...
    def draw(self):
        ...
        if perception_active:self._draw_perception_fog(selected_entity,perception_field)
        else:self.teacher_visibility.fill(1)
