...
from .session_save import _restore_society,_society_payload
...
class PersistentRegionStore:
    ...
    def save(self, key: RegionKey, world, *, exclude_entity_id: int | None = None, society=None) -> dict:
        ...
        if society is not None:
            ...
            payload={"format":"nullvector-region-civilization/1.0.0","atlas_seed":self.atlas_seed,"region":[key.x,key.y,key.depth],"world_sha256":report["sha256"],"society":society_payload};payload["semantic_sha256"]=hashlib.sha256(json.dumps(payload,sort_keys=True,separators=(",",":"),allow_nan=False).encode()).hexdigest();path=self.civilization_path(key);path.parent.mkdir(parents=True,exist_ok=True);temporary=path.parent/("."+path.name+".tmp-"+uuid.uuid4().hex);temporary.write_text(json.dumps(payload,sort_keys=True,separators=(",",":"),allow_nan=False)+"\n",encoding="utf-8");os.replace(temporary,path)
        return {**report, "region": (key.x, key.y, key.depth), "excluded": exclude_entity_id,"civilization":society is not None}
    ...
    def load(self, key: RegionKey, *, motion_policy=None, behavior_policy=None, colony_policy=None, society_policy=None, include_society:bool=False):
        ...
        if not civilization.is_file():return world,None
        ...
        return world,_restore_society(payload["society"],world,society_policy)
