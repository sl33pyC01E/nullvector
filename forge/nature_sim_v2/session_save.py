...
def _restore_society(payload: dict, world) -> SocietyLayer:
    ...
    for item in payload["settlements"]:
        settlement = SettlementState(item["settlement_id"], item["faction_id"], tuple(item["center"]), int(item["population"]), float(item["wealth"]), float(item["food"]), float(item["power"]), [_building(value) for value in item["buildings"]], {tuple(value) for value in item["roads"]}, int(item["founded_tick"]))
